...
│	
│	=======================================================
│	Authorship:	neoLIZV, https://github.com/numbBrain
...
suppliedLinks = []
#//////////////////////////////////////////////////
def importParser(links:str, read:str, dest:str, unzip=True, retain_zip=True):
	try:
		arguments.links = links
		arguments.read = read
		arguments.dest = dest
		arguments.unzip = unzip
		arguments.retain_zip = retain_zip
		#//////////////////////////////////////////////////
		# destination folder supplied by the user
		destination = arguments.dest
		print(f"✅ Specified download location: {destination}")

		#check if unzip argument is supplied
		if arguments.unzip:
			if arguments.retain_zip:
				print(f"✅ zipfiles will not be deleted after unzipping")
			elif not arguments.retain_zip:
				print(f"✅ zipfiles will be deleted after unzipping")

		#//////////////////////////////////////////////////
		#iterate thru' links in the list
		if arguments.read is None:
			suppliedLinks = [arguments.links]
		else:
		...
		for suppliedLink in suppliedLinks:
			...
			print(f"✅ Downloading from URL : {suppliedLink}")
			...
			with requests.Session() as session:
					...
					with open(tempFilePath, "wb") as zipFile: #write file to disk
						...
						for chunk in zipFileResp.iter_content(chunk_size=2**20):
							...
							if chunk:
									...
									print(f"{progress}% of {formattedZipFileSize} at {speed}ps ETA {eta}", end="\r", flush=True)

				except KeyboardInterrupt:
					print(f"{progress}% of {formattedZipFileSize} at {speed} ps ETA {eta}")
				...
				if os.stat(tempFilePath).st_size == zipFileSize:
					...
					print(f"✅ 100% of {formattedZipFileSize} in {calcElaspedTime(startTime, time.time())}")
